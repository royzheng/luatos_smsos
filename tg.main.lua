...
-- TG chat id
luatsms.tg_chat_id = ""
-- 比如：https://api.telegram.org/bot<YourBOTToken>/sendMessage
-- 鉴于众所周知的原因，上面的api地址并不能访问
-- 你应该在CF worker上绑定一个域名来反向代理这个api, worker的js见tg.worker.js(记得修改下whitelist)
-- 例如你绑定了yourdomain.com，那么这里的api就是https://yourdomain.com/bot<YourBOTToken>/sendMessage
luatsms.tg_api_url = ""
-- 可以远程控制发送短信的手机号名单 比如 luatsms.control_phones = {"13512345678", "13900000000"}
...
luatsms.db = {
    ...
    is_control_phone = function(phone)
        for i=1, #(luatsms.control_phones) do      
            if phone:endsWith(control_phones[i]) then
                return true
            end
        end
        return false
